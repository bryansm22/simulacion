...
//SE COMPILA CON g++ nombre -lm
//Se corre con ./a.out pi
...
void input () {
    ...
    fscanf (fp, "%d %lf %lf %lf", &aux, &x[inod], &y[inod], &z[inod]);
    printf("Los nodos son; %lf %lf %lf", x[inod], y[inod], z[inod]);
    }    
    for (i=0; i<6; i++) jumpline (fp);
    /* reading connectivities */
    for (ielem=1; ielem<=Nelem; ielem++){
        fscanf (fp, "%d", &aux);
        printf("ESto trae el primer escan %d", aux);
        for(j=0;j<4;j++){ 
            fscanf (fp, "%d", &N[(ielem-1)*3+j]);
            printf("Esto trae las conectividades %d", N[(ielem-1)*3+j]);
        }
